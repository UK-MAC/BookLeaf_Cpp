        ...
        DataControl &data)
{
    ...
    auto store7    = data[DataID::ALE_STORE7].device<double, VarDim>();
    auto store8    = data[DataID::ALE_STORE8].device<double, VarDim>();
    ...
    kernel::sumFlux(id1, id2, sizes.nel, sizes.nel1, elel, elfc, fcdv, store7);
    ...
    kernel::sumFlux(id1, id2, sizes.nel, sizes.nel1, elel, elfc, fcdm, store8);

    // Update
    kernel::updateBasisEl(ale.global->zerocut, ale.global->dencut, store7,
            store8, store5, store6, store1, store2, store3, elvolume, elmass,
            eldensity, sizes.nel);
}
        ...
        DataControl &data)
{
    ...
    kernel::updateEl(id1, id2, sizes.nel, sizes.nel1, elel, elfc, store2,
            elmass, store6, flux, store7, elenergy);
}
        ...
        DataControl &data)
{
    ...
    kernel::updateNd(sizes.nnd, sizes.nel1, sizes.nnd1, elnd, ndeln, ndelf,
            ndel, store2, store1, store6, active, flux, store7, ndu);
    ...
    kernel::updateNd(sizes.nnd, sizes.nel1, sizes.nnd1, elnd, ndeln, ndelf,
            ndel, store2, store1, store6, active, flux, store7, ndv);
}
