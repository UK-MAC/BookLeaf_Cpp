...
/**
 * CUDA launches are asynchronous, so the kernel timing metrics are useless.
 * This routine blocks until kernel completion. If the timing information is
 * required, uncomment the body. Also useful for debugging, as this checks if an
 * error occurred in the kernel.
 */
inline void
cudaSync()
{
#ifdef BOOKLEAF_KOKKOS_CUDA_SUPPORT
    // Can comment this out to slightly increase overlap at the expense of the
    // timers being wrong
    Kokkos::Cuda::fence();
#endif
}
