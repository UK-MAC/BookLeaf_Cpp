    ...
    for (int iside = 0; iside < NFACE / 2; iside++) {
                ...
                [=] __device__ (int const iel)
        {
            // XXX(timrlaw): Manually fused these three loops
            {
                ...
                double den = sqrt(fw1*fw1 + fw2*fw2);
                tmp = den > zerocut ? den : zerocut;
                den = 1.0 / tmp;
                double uhat = fw1 * den;
                double vhat = fw2 * den;

                den = sqrt(fw3*fw3 + fw4*fw4);
                tmp = den > zerocut ? den : zerocut;
                den = 1.0 / tmp;
                double xhat = fw3 * den;
                double yhat = fw4 * den;
                ...
                fw1 = (fw1*uhat + fw2*vhat) / copysign(tmp, den);
                tmp = fabs(fw1);
                tmp = tmp > zerocut ? tmp : zerocut;
                fw1 = 1.0 / copysign(tmp, fw1);
                ...
                tmp = fabs(den);
                tmp = tmp > zerocut ? tmp : zerocut;
                fw2 = (du(in1, ins)*uhat + dv(in1, ins)*vhat) / copysign(tmp, den);
                ...
                tmp = fabs(den);
                tmp = tmp > zerocut ? tmp : zerocut;
                fw3 = (du(in2, ins)*uhat + dv(in2, ins)*vhat) / copysign(tmp, den);
            ...
            // Apply limiter
            {
                ...
                double w1 = cvisc1 * sqrt(0.5 * (elcs2(iel) + elcs2(limins)));
                ...
                limins = elel(iel, is2);
                w1 = cvisc1 * sqrt(0.5 * (elcs2(iel) + elcs2(limins)));
                w2 = scratch(iel, 2);
                w3 = scratch(iel, 3);
        ...
        int nel)
{
            ...
            [=] __device__ (int const iel)
    {
        ...
        for (int iside = 0; iside < NFACE; iside++) {
            ...
            double w7 = sqrt((w2-w3)*(w2-w3) + (w5-w6)*(w5-w6));
            double w8 = sqrt(w1*w1 + w4*w4);
